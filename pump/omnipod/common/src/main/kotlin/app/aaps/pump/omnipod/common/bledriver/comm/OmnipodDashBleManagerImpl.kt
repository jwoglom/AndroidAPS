        ...
        .create { emitter ->
            ...
            try {
                ...
                if (preferences.get(DashBooleanPreferenceKey.UseBonding)) {
                    val priorBondStateName = bondStateName(podDevice.bondState)
                    if (podDevice.bondState == BluetoothDevice.BOND_NONE) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.VANILLA_ICE_CREAM && ActivityCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) == PackageManager.PERMISSION_GRANTED) {
                            DashMetrics.setLifecycle("bonding")
                            val tBondStart = System.nanoTime()
                            var bondOutcome = "failed"
                            try {
                                val result = podDevice.createBond()
                                aapsLogger.debug(LTag.PUMPBTCOMM, "Bonding with pod resulted $result")
                                Thread.sleep(10000)
                                bondOutcome = when (podDevice.bondState) {
                                    BluetoothDevice.BOND_BONDED  -> "bonded"
                                    BluetoothDevice.BOND_BONDING -> "timeout"
                                    else                         -> "failed"
                                }
                            } finally {
                                DashMetrics.bondPhase(
                                    priorBondState = priorBondStateName,
                                    durationMs = (System.nanoTime() - tBondStart) / 1_000_000L,
                                    outcome = bondOutcome,
                                    useBondingPref = true
                                )
                            }
